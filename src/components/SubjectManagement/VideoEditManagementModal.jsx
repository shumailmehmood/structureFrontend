...
import { editVideoDocument } from '../../api/api';
const VideoManagementModal = props => {
    ...
    const { handleSubmit, register, errors } = useForm({
        defaultValues: {
            ...
            videoUrl: props.doc.original.videoUrl,
            thumbnailUrl: props.doc.original.thumbUrl,
        }
    });
    const onSubmit = async (values) => {
        setSpinner(true)
        let response = await editVideoDocument(props.doc.value,values.name, values.videoUrl, values.thumbnailUrl);
        if (response.error) {
            setSpinner(false)
            props.handleClose()
        } else {
                            ...
                            <Modal show={props.show} onHide={props.handleClose}>
                                <Form onSubmit={handleSubmit(onSubmit)}>
                                    <Modal.Header closeButton className="mdhead" style={{ backgroundColor: '#0F4B5F', color: '#E1FADF' }}>
                                        <Modal.Title><center>Edit Video</center></Modal.Title>
                                    </Modal.Header>
                                    <Modal.Body>
                                        <FormGroup
